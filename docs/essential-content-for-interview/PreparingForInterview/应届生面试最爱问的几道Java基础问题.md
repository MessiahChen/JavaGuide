...
4. **因此，equals 方法被覆盖过，则 hashCode 方法也必须被覆盖**（可以这样认为：根据第一条规定，两个对象如果equals，必然有它们的hashcode相同。因此原先是按对象地址来实现equals和hashcode，如果重写了equals使其按照对象内容来判断，必须重写hashcode使其也按对象内容来获取哈希值，这样两个对象equal也必然有它们的hashcode相同）
